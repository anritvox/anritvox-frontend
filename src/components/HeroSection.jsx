...
export default function HeroSection() {
  ...
  useEffect(() => {
    ...
    return () => setInterval(() => clearInterval(timer), 0);
  }, [isPaused]);
    ...
    <div className="relative h-[85vh] min-h-[600px] w-full overflow-hidden bg-slate-950 group">
      ...
      <div className="relative z-20 container mx-auto px-6 h-full flex flex-col justify-center">
        <div className="max-w-3xl">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-olive-400/10 border border-olive-400/20 text-olive-400 text-xs font-bold uppercase tracking-[0.2em] mb-6 animate-fade-in">
            <Zap size={14} />
          ...
          <h1 className="text-6xl md:text-8xl font-black text-white leading-tight mb-6 tracking-tighter">
            {slides[current].title.split(',').map((part, i) => (
              <span key={i} className="block last:text-transparent last:bg-clip-text last:bg-gradient-to-r last:from-olive-300 last:to-olive-500">
                {part}
              </span>
            ))}
          </h1>
          ...
          <div className="flex flex-wrap gap-4">
            <Link
              ...
              className="px-8 py-4 bg-olive-400 hover:bg-white text-slate-950 font-black uppercase tracking-wider rounded-2xl transition-all duration-300 flex items-center gap-3 shadow-[0_0_30px_rgba(128,141,100,0.3)] hover:shadow-[0_0_40px_rgba(255,255,255,0.2)]"
            >
              <ShoppingBag size={20} />
              {slides[current].cta}
            </Link>
      ...
      <div className="absolute bottom-12 right-12 z-30 flex items-center gap-6">
          ...
          {slides.map((_, i) => (
            <button
              ...
              className={`h-1.5 transition-all duration-500 rounded-full ${
                i === current ? 'w-12 bg-olive-400' : 'w-3 bg-slate-700 hover:bg-slate-500'
              }`}
            />
          ))}
        </div>
        ...
        <div className="flex gap-3">
          <button
            ...
            className="p-4 rounded-2xl border border-slate-800 bg-slate-900/50 backdrop-blur-xl text-white hover:bg-olive-400 hover:text-slate-950 hover:border-olive-400 transition-all duration-300"
          >
            <ChevronLeft size={24} />
          </button>
          <button
      ...
      <div className="absolute bottom-0 left-0 w-full z-20 border-t border-slate-800/50 bg-slate-950/50 backdrop-blur-xl py-6 hidden lg:block">
        <div className="container mx-auto px-6 flex justify-between items-center text-slate-400 font-bold uppercase tracking-widest text-[10px]">
          ...
          <div className="flex items-center gap-3">
            <Globe size={18} className="text-olive-400" />
