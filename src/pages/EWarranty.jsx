...
//   // Step 1: Validate the serial number (Existing Logic)
...
//   // Step 2: Register the warranty (Existing Logic)
...
//     "w-full bg-lime-700 hover:bg-lime-800 text-white font-bold py-3 px-6 rounded-full shadow-lg transform hover:scale-105 transition-all duration-300 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed"; // Added disabled styles
...
//               Product:{" "}
//               <strong className="text-lime-700">{info.product_name}</strong>
//             </p>
//             <input
//               type="text"
...
//                 setLoading(false);
//               }}
//               // Go back button styled for white theme
...
//             <svg
//               className="mx-auto h-16 w-16 text-green-600" // Green shade adjusted for better contrast on white
//               fill="none"
//               stroke="currentColor"
//               viewBox="0 0 24 24"
...
export default function EWarranty() {
  ...
  const handleValidate = async () => {
    ...
    try {
      const data = await validateSerial(serial);
      setInfo(data); // Set the product info here (should include product_name and product_image_url)
      ...
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl border border-gray-200 p-8 space-y-6 animate-fade-in">
        ...
        {step === 2 && info && (
          <div className="space-y-6 animate-fade-in">
            ...
            <input
              type="tel"
              placeholder="Your 10-Digit Phone Number (Optional)"
              value={form.user_phone}
              onChange={(e) =>
                setForm({
                  ...form,
            ...
            <button
              ...
              className={commonButtonClasses}
              disabled={
                loading || !form.user_name.trim() || !form.user_email.trim()
              }
            >
              {loading ? "Registering..." : "Submit Registration"}
            </button>
