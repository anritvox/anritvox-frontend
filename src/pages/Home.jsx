  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        ...
        // FIX: Filter Banners by Position
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] font-sans text-gray-900 overflow-x-hidden relative selection:bg-olive-500 selection:text-white">
      ...
      <div className="relative max-w-[1600px] mx-auto lg:mt-6 lg:px-6 z-10">
        <div className="relative w-full h-[65vh] lg:h-[75vh] lg:rounded-[2.5rem] overflow-hidden shadow-2xl shadow-gray-200/50 bg-gray-900">
          {loading ? (
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <div className="absolute inset-0 bg-gradient-to-r from-gray-900/90 via-gray-900/40 to-transparent z-10 pointer-events-none" />
                
                {/* Advanced Slider Image Animation */}
                <motion.img 
                ...
                <div className="relative z-20 w-full px-6 md:px-16 lg:px-24">
                  ...
                  >
                    ...
                    <Link to={currentBanner.link_url || currentBanner.link || "/shop"} className="inline-flex items-center gap-3 px-8 py-4 bg-white text-gray-900 rounded-2xl font-bold hover:bg-olive-500 hover:text-white transition-all duration-300 group">
      ...
      {/* App-Style Category Scroll with Animation */}
      ...
      {promoBanners.length > 0 && !loading && (
        <section className="max-w-[1400px] mx-auto px-4 lg:px-6 pb-12 relative z-20">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {promoBanners.map((promo, idx) => (
              <motion.div
                ...
                className="relative rounded-[2rem] overflow-hidden shadow-lg group h-48 md:h-64 cursor-pointer"
              >
                <Link to={promo.link_url || promo.link || "/shop"} className="block w-full h-full relative">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-gray-900/90 via-gray-900/30 to-transparent flex flex-col justify-end p-8">
                    <h3 className="text-white text-2xl md:text-3xl font-black tracking-tight" dangerouslySetInnerHTML={{ __html: promo.title }}></h3>
                    {promo.subtitle && <p className="text-olive-400 font-bold tracking-wider text-sm mt-2 uppercase">{promo.subtitle}</p>}
                    {promo.description && <p className="text-gray-300 text-sm mt-2 line-clamp-2 max-w-md">{promo.description}</p>}
                  </div>
                </Link>
              </motion.div>
            ))}
      ...
      {/* Premium Product Grid with Hover Animations */}
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 md:gap-6">
          {loading ? (
            ...
            <motion.div 
              ...
              className="group bg-white rounded-3xl p-3 md:p-5 border border-gray-100/80 shadow-[0_2px_10px_rgba(0,0,0,0.02)] hover:shadow-[0_20px_40px_rgba(0,0,0,0.06)] hover:border-olive-200 transition-all duration-500 relative flex flex-col h-full overflow-hidden"
            >
              ...
              <Link to={`/product/${product._id || product.id}`} className="block relative aspect-square mb-5 bg-[#F8F9FA] rounded-2xl overflow-hidden group-hover:bg-olive-50/30 transition-colors">
                {/* Product Image Advanced Hover Scale */}
                <motion.img 
