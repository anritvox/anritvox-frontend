...
export default function WarehouseLogin() {
  ...
  const handleRequestOtp = async (e) => {
    ...
    setLoading(true);
    try {
      // Hits your backend warehouse OTP requester
  ...
  const handleVerifyOtp = async (e) => {
    ...
    setLoading(true);
    try {
      // Verifies OTP and retrieves the warehouseToken
      const res = await api.post('/auth/warehouse/verify-otp', { email, otp });
      
      if (res.data.token) {
        localStorage.setItem('warehouseToken', res.data.token);
        // Also set standard token so the IFrame bridge passes it successfully
