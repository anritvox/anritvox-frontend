            ...
            <div>
              ...
              <div className="relative">
                <input 
                  ...
                  className="w-full bg-slate-50 border-none px-4 py-3 text-sm focus:ring-2 focus:ring-emerald-500 transition-all"
                  value={searchTerm}
              ...
              <div className="space-y-2">
                {dynamicCategories.map(cat => (
                  <button 
                    ...
                    className={`block w-full text-left px-3 py-2 text-sm transition-all ${selectedCategory === cat ? 'bg-emerald-500 text-black font-bold' : 'text-slate-600 hover:bg-slate-50'}`}
                  >
                    {cat}
                  </button>
                ))}
              ...
              <div className="space-y-2">
                {priceRanges.map(range => (
                  <button 
                    ...
                    className={`block w-full text-left px-3 py-2 text-sm transition-all ${priceRange === range ? 'bg-emerald-500 text-black font-bold' : 'text-slate-600 hover:bg-slate-50'}`}
                  >
                    {range}
                  </button>
                ))}
              ...
              <div className="space-y-2">
                {brands.map(brand => (
                  <button 
                    ...
                    className={`block w-full text-left px-3 py-2 text-sm transition-all ${selectedBrand === brand ? 'bg-emerald-500 text-black font-bold' : 'text-slate-600 hover:bg-slate-50'}`}
                  >
                    {brand}
                  </button>
                ))}
          ...
          <main className="flex-1">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 mb-8 pb-6 border-b border-slate-100">
              <div className="text-sm text-slate-500">
                Displaying <span className="text-slate-900 font-bold">{filteredProducts.length}</span> units found in sector
              </div>
            ...
            {filteredProducts.length > 0 ? (
              ...
              <div className="text-center py-20 border-2 border-dashed border-slate-100">
                ...
                <button 
                  onClick={() => { setSearchTerm(""); setSelectedCategory("All"); setSelectedBrand("All"); setPriceRange("All"); }}
                  className="mt-6 text-emerald-500 font-bold uppercase tracking-widest text-sm hover:underline"
                >
                  Reset Nexus
                </button>
              </div>
