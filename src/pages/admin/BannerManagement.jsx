...
export default function BannerManagement({ token }) {
  ...
  const loadBanners = async () => {
    ...
    setError('');
    try {
      const data = await fetchBannersAdmin(token);
    ...
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-bold text-cyan-400 flex items-center gap-2">
          <FiImage /> Banner Management
        </h2>
        <button onClick={loadBanners} className="flex items-center gap-1 text-xs text-gray-400 hover:text-white border border-white/10 px-3 py-1.5 rounded-lg hover:bg-white/5">
          <FiRefreshCw size={12} /> Refresh
        </button>
      </div>

      {error && (
        <div className="flex items-center gap-2 bg-red-500/10 text-red-400 border border-red-500/20 rounded-lg p-3 mb-4 text-sm">
          <FiAlertCircle size={14} /> {error}
        </div>
      )}
      {success && (
        <div className="flex items-center gap-2 bg-green-500/10 text-green-400 border border-green-500/20 rounded-lg p-3 mb-4 text-sm">
          {success}
        </div>
      )}
      ...
      <div className="bg-white/5 border border-white/10 rounded-xl p-5 mb-6">
        <h3 className="font-semibold text-white mb-4">{editingId ? 'Edit Banner' : 'Add New Banner'}</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className={`text-lg ${form.is_active ? 'text-green-400' : 'text-gray-500'}`}
            >
