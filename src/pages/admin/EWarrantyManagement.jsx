  ...
  const fetchData = async () => {
    ...
    try {
      const [resW, resS, resP] = await Promise.all([
        ...
        api.get("/serials/admin/all"), // Pointing directly to the high-performance administrative system endpoint
        productsApi.getAllAdmin()
      ]);
      setWarranties(resW.data?.data || resW.data || []);
      

      const vaultData = resS.data?.serials || (Array.isArray(resS.data) ? resS.data : []);
      setSerialsVault(vaultData);
  ...
  const handleGenerate = async () => {
    ...
    try {
      await api.post("/serials/generate", {
        ...
        month: genData.month ? parseInt(genData.month, 10) : undefined,
        year: genData.year ? parseInt(genData.year, 10) : undefined
      });
                ...
                <tbody className="divide-y divide-slate-800/50">
                  {filteredVault.length === 0 ? (
                    ...
                    <tr key={s.id} className="hover:bg-purple-500/[0.02] transition-colors group">
                      ...
                      <td className="p-6 text-right">
                        <button 
                          onClick={() => handleDeleteSerial(s.id, s.product_id)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-xl">
          <div className="bg-[#0a0c10] border border-slate-800 w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-8 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Custom Month (Optional)</label>
                  <select 
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-xs font-bold text-white outline-none focus:border-purple-500/50 appearance-none cursor-pointer"
                    value={genData.month} onChange={e => setGenData({...genData, month: e.target.value})}
                  >
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Custom Year (Optional)</label>
                  <select 
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-xs font-bold text-white outline-none focus:border-purple-500/50 appearance-none cursor-pointer"
                    value={genData.year} onChange={e => setGenData({...genData, year: e.target.value})}
                  >
